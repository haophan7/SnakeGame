...
def draw_obstacles(screen, obstacles):
    for obstacle in obstacles:
        ...
        pygame.draw.rect(screen, PINK, obstacle_rect)
...
def display_victory_message(screen, score):
    font = pygame.font.SysFont("monospace", 36)
    text1 = font.render("Congratulations! You won Level 2!", True, GRAY)
...
def main():
    ...
    while running:
        ...
        obstacles_left = [[0, y] for y in range(40, SCREEN_HEIGHT - 480, CELL_SIZE)]
        obstacles_left2 = [[0, y] for y in range(520, SCREEN_HEIGHT, CELL_SIZE)]
        obstacles_right = [[SCREEN_WIDTH - CELL_SIZE, y] for y in range(40, SCREEN_HEIGHT - 480, CELL_SIZE)]
        obstacles_right2 = [[SCREEN_WIDTH - CELL_SIZE, y] for y in range(520, SCREEN_HEIGHT, CELL_SIZE)]
        obstacles_top = [[x, 40] for x in range(0, SCREEN_WIDTH - 720, CELL_SIZE)]
        obstacles_top2 = [[x, 40] for x in range(720, SCREEN_WIDTH, CELL_SIZE)]
        obstacles_bottom = [[x, SCREEN_HEIGHT - CELL_SIZE] for x in range(0, SCREEN_WIDTH - 720, CELL_SIZE)]
        obstacles_bottom2 = [[x, SCREEN_HEIGHT - CELL_SIZE] for x in range(720, SCREEN_WIDTH, CELL_SIZE)]
        obstacles_center = [[x, 220] for x in range(80, SCREEN_WIDTH - 80, CELL_SIZE)]
        obstacles_center2 = [[x, 400] for x in range(80, SCREEN_WIDTH - 80, CELL_SIZE)]
        obstacles = obstacles_left + obstacles_right + obstacles_top + obstacles_bottom + obstacles_top2 + obstacles_left2 + obstacles_right2 + obstacles_bottom2 + obstacles_center + obstacles_center2
        ...
        victory_message_displayed = False  # Biến để kiểm tra xem thông báo chiến thắng đã được hiển thị chưa
                ...
                if snake[0] in food_positions:
                    ...
                    if check_speed_limit(snake, food_positions, orange_positions, FPS):
                        ...
                        if black_food_eaten >= 1:
                            if not victory_message_displayed:
                                display_victory_message(screen, score)
                                victory_message_displayed = True
                                # Chờ 10 giây trước khi chuyển sang cấp độ 2
                                pygame.time.wait(10000)
                                # Đóng cửa sổ hiện tại trước khi chuyển sang màn mới
                                pygame.display.quit()
                                # Chuyển sang màn chơi mới
                                subprocess.run(["python", "campaign3.py"])
                            break
                    else:
